...
class SimonScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Title
    this.titleText = this.add.text(width / 2, 60, "Simon Game", {
      fontSize: "40px",
      fontStyle: "bold",
      color: "#a78bfa",
    }).setOrigin(0.5);
    ...
    this.scoreText = this.add.text(width / 2, 150, "", {
      fontSize: "22px",
      color: "#4ade80",
    }).setOrigin(0.5);
    ...
    this.gridX =
      width / 2 -
      (GRID_SIZE * CELL_SIZE + (GRID_SIZE - 1) * GAP) / 2;
  ...
  handleCellClick(index) {
    ...
    if (this.playerMoves.length === this.sequence.length) {
      this.playerMoves = [];
      this.level++;
      this.scoreText.setText("Score: " + this.level);
  ...
  gameOver() {
    this.infoText.setText("❌ Wrong! Restarting...");
    this.started = false;

    this.time.delayedCall(1000, () => {
      this.cells.forEach(c => c.setVisible(false));
      this.playButton.setVisible(true);
      this.infoText.setText("Click Play to Start");
